...
waitForElement(
  [".main-navBar-mainNav .os-viewport.os-viewport-native-scrollbars-invisible"],
  ([scrollNode]) => {
...
function legacy() {
  ...
  waitForElement(
    [".Root__right-sidebar .LayoutResizer__input"],
    ([resizer]) => {
      const observer = new MutationObserver(updateVariable);
  ...
  waitForElement(
    [
      ".main-navBar-navBar .os-viewport.os-viewport-native-scrollbars-invisible",
    ],
    ([scrollNode]) => {
      scrollNode.setAttribute("fade", "bottom");
      scrollNode.addEventListener("scroll", () => {
  ...
  waitForElement(
    ...
    ([root, firstItem]) => {
      ...
      function loadPlaylistImage() {
        for (const item of listElem.children) {
          ...
          if (app === "playlist") {
            ...
            img.src =
              base64 ||
              "https://cdn.jsdelivr.net/gh/spicetify/spicetify-themes@master/Dribbblish/images/tracklist-row-song-fallback.svg";
            continue;
          }
      ...
      new MutationObserver(loadPlaylistImage).observe(listElem, {
        childList: true,
      });
    },
  );
